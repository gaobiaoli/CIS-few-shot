...
dataloader_shot,dataloader_val,dataloader_test=get_dataloader(type='mocs',train_tranform=train_tranform)
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=epoches//3, gamma=0.1)

for epoch in range(0, epoches):
    loss1, acc1 = train_model(model, dataloader_shot, loss_fn, optimizer, epoch)
    if epoch==epoches-1 or acc1==100.0:
