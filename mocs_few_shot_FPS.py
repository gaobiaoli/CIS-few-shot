...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, required=False, default="ViT-B/16",help="The name of the model (e.g., 'vit_l/14')")
    ...
    dataloader_test = DataLoader(dataset=dataset_test, num_workers=12, batch_size=1)
    
    t1=time.time()
    all_predictions, all_targets, (accuracy, precision, recall, f1) = clip_adapter.eval(
        adapt=True,dataloader=dataloader_test
    )
    total_time=time.time()-t1
    print("\n**** Few-shot CLIP's val accuracy: {:.2f}. FPS:{:.2f}****\n".format(accuracy * 100,len(all_targets)/total_time))